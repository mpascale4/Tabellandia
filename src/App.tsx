...
const getAdventureWorldProgress = (profile: UserProfile, worldId: number, devModeEnabled: boolean) => {
  const base = profile.worldProgress[worldId] || {
    ...
    highScore: 0,
    stars: 0,
    coins: worldId === 2 ? (profile.coins || 0) : 0,
    lightDrops: worldId === 2 ? (profile.lightDrops || 0) : 0,
  };

  const withCurrencies = {
    ...base,
    coins: base.coins ?? (worldId === 2 ? (profile.coins || 0) : 0),
    lightDrops: base.lightDrops ?? (worldId === 2 ? (profile.lightDrops || 0) : 0),
  };

  return devModeEnabled
    ? { ...withCurrencies, completedSteps: [...ALL_STEP_IDS] }
    : withCurrencies;
};
...
export default function App() {
  ...
  const [showAllKingdomsCelebrationModal, setShowAllKingdomsCelebrationModal] = useState<boolean>(false);
  ...
  const activeWorldId = useMemo(() => {
    if (!profile) return 2;
    const active = WORLDS_DATA.find(w => {
      const isUnlocked = devModeEnabled || profile.unlockedWorlds.includes(w.id);
      if (!isUnlocked) return false;
      const worldProg = getAdventureWorldProgress(profile, w.id, devModeEnabled);
      const stepsCount = worldProg.completedSteps.length;
      const rebuiltCount = devModeEnabled ? w.monuments.length : worldProg.rebuiltMonuments.length;
      const isCompleted = stepsCount === ALL_STEP_IDS.length && rebuiltCount === w.monuments.length;
      return !isCompleted;
    });
    if (active) return active.id;
    const unlocked = WORLDS_DATA.filter(w => devModeEnabled || profile.unlockedWorlds.includes(w.id));
    return unlocked.length > 0 ? unlocked[unlocked.length - 1].id : 2;
  }, [profile, devModeEnabled]);

  useEffect(() => {
    if (activeTab === 'adventure' && selectedWorldId === null && profile && !showProfilePicker) {
      const scrollTarget = () => {
        const activeEl = document.getElementById(`active-world-card-${activeWorldId}`);
        if (activeEl) {
          ...
          while (parent) {
            if (parent.classList.contains('overflow-y-auto') || parent.scrollHeight > parent.clientHeight) {
              ...
              const targetScrollTop = parent.scrollTop + relativeTop - (parent.clientHeight / 2) + (elRect.height / 2);
              if (Math.abs(parent.scrollTop - targetScrollTop) > 10) {
      ...
      const t1 = setTimeout(scrollTarget, 150);
      const t2 = setTimeout(scrollTarget, 500);
      const t3 = setTimeout(scrollTarget, 1000);
      const t4 = setTimeout(scrollTarget, 1600);
      return () => {
        clearTimeout(t1);
        clearTimeout(t2);
        clearTimeout(t3);
  ...
  }, [activeTab, selectedWorldId, activeWorldId, profile, showProfilePicker]);
  ...
  useEffect(() => {
    if (activeTab !== 'adventure' || selectedWorldId !== null) {
  ...
  // Auto-unlock del regno successivo quando il regno precedente ha la Sfida completata o tutti i passi svolti
  useEffect(() => {
    if (!profile) return;
    let changed = false;
    const nextUnlocked = [...profile.unlockedWorlds];

    WORLDS_DATA.forEach(w => {
      const wp = profile.worldProgress[w.id];
      if (wp) {
        const isSfidaDone = wp.completedSteps?.includes('sfida');
        const allStepsDone = wp.completedSteps?.length >= 6;
        if ((isSfidaDone || allStepsDone) && w.id + 1 <= 9 && !nextUnlocked.includes(w.id + 1)) {
          nextUnlocked.push(w.id + 1);
          changed = true;
        }
      }
    });
  ...
  return (
    <div className="min-h-screen w-full bg-slate-900 flex flex-col items-center justify-center p-2 md:p-4 select-none">
              ...
              <>
                ...
                <div className={`flex flex-col justify-center gap-1 shrink-0 bg-white/65 rounded-2xl border border-white/80 ${isPhoneMode ? 'px-2 py-1 min-w-[72px]' : 'px-3 py-1.5 min-w-[105px]'}`}>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      const currentWorld = selectedWorldId ? WORLDS_DATA.find(w => w.id === selectedWorldId) : null;
                      const isRestored = currentWorld
                        ? (() => {
                            const wProg = getAdventureWorldProgress(profile, currentWorld.id, devModeEnabled);
                            const count = devModeEnabled ? currentWorld.monuments.length : (wProg?.rebuiltMonuments?.length || 0);
                            return count >= currentWorld.monuments.length;
                          })()
                        : false;
                    ...
                    className={`flex items-center justify-between gap-1.5 border-t border-sky-950/10 pt-0.5 rounded px-1 transition-colors text-left w-full group ${
                      selectedWorldId && (() => {
                        ...
                        const count = devModeEnabled ? currentWorld.monuments.length : (wProg?.rebuiltMonuments?.length || 0);
                        return count >= currentWorld.monuments.length;
                      })() ? 'cursor-default opacity-80' : 'hover:bg-sky-100/50 cursor-pointer'
                    }`}
                    title={
                      selectedWorldId && (() => {
                        const currentWorld = WORLDS_DATA.find(w => w.id === selectedWorldId);
                        ...
                        const count = devModeEnabled ? currentWorld.monuments.length : (wProg?.rebuiltMonuments?.length || 0);
                        return count >= currentWorld.monuments.length;
                      })() ? 'Tutti i monumenti del regno sono eretti!' : 'Tocca per scoprire a cosa servono le Gocce'
                    }
                  >
                ...
                >
                  ...
                  {activeTab === 'adventure' && (
                    <div className="space-y-6">
                      {profile && (() => {
                        const areAllKingdomsErected = WORLDS_DATA.every(world => {
                          ...
                          return rebuiltCount >= world.monuments.length;
                        });

                        if (!areAllKingdomsErected) return null;

                        return (
                          <motion.div
                            initial={{ scale: 0.95, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
                            className="rounded-3xl border-4 border-amber-400 bg-gradient-to-r from-amber-500 via-emerald-600 to-indigo-700 p-6 text-white text-center shadow-2xl relative overflow-hidden"
                          >
                            <div className="absolute inset-0 opacity-25 bg-[radial-gradient(circle,_var(--tw-gradient-stops))] from-yellow-300 via-transparent to-transparent"></div>
                            <div className="relative z-10 space-y-3">
                              <div className="flex items-center justify-center gap-2 text-3xl sm:text-4xl">
                                <span>👑</span><span>🎆</span><span>🏆</span><span>🏛️</span><span>✨</span>
                              </div>
                              <h3 className="text-xl sm:text-2xl font-black tracking-tight">
                                IMPRESA LEGGENDARIA! HAI ERETTO TUTTI I REGNI!
                              </h3>
                              <p className="text-xs sm:text-sm text-amber-100 max-w-xl mx-auto font-medium leading-relaxed">
                                Hai ricostruito ogni monumento in tutti e 9 i Regni di Tabellandia! La nebbia si è dissolta e l'armonia è tornata ovunque. Sei un vero Maestro delle Tabelline!
                              </p>
                              <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
                                <button
                                  ...
                                  className="px-6 py-3 rounded-2xl bg-gradient-to-r from-amber-300 to-yellow-400 text-amber-950 font-black text-sm shadow-xl hover:scale-105 transition-all cursor-pointer inline-flex items-center gap-2 border-2 border-amber-200"
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    sound.playLevelUp();
                                    setShowFireworks(true);
                                    setShowAllKingdomsCelebrationModal(true);
                                  }}
                                  className="px-6 py-3 rounded-2xl bg-white/20 hover:bg-white/30 text-white font-black text-sm shadow-lg backdrop-blur-md transition-all cursor-pointer inline-flex items-center gap-2 border border-white/40"
                                  id="celebration-continue-btn"
                                >
                                  <span>📜 Leggi il Messaggio Motivazionale</span>
                                </button>
                              </div>
                            </div>
                          </motion.div>
                      ...
                      <SurfaceCard tone="soft" padding={isPhoneMode ? 'sm' : 'md'} className={`text-center ${isPhoneMode ? '' : 'md:text-left'}`}>
                        ...
                        <div className="mt-4 flex flex-wrap gap-2 text-[10px] font-bold text-sky-900/80 sm:text-[11px]">
                          <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-emerald-800 sm:px-3 sm:py-1">🏛️ Regno Ripristinato</span>
                      ...
                      <ResponsiveGrid columns={1} variant="cards" className="items-stretch max-w-2xl mx-auto">
                        {WORLDS_DATA.map(world => {
                          ...
                          const isActiveWorld = isUnlocked && !isCompleted;
                          const statusLabel = !isUnlocked ? 'Bloccato' : isCompleted ? 'Regno Ripristinato' : 'In corso (Attivo)';

                          return (
                            <div
                              key={world.id}
                              id={`active-world-card-${world.id}`}
                              className={`w-full overflow-hidden text-left rounded-3xl border-2 p-3 sm:p-4 shadow-lg transition-all ${
                                !isUnlocked
                                  ? 'bg-slate-200 border-slate-300 text-slate-600'
                                  : isCompleted
                                    ? 'bg-gradient-to-br from-emerald-300 via-emerald-200 to-white border-emerald-500 text-emerald-950'
                                    : 'bg-gradient-to-br from-amber-50 via-sky-50 to-amber-100 border-amber-400 text-sky-950 animate-active-world'
                              }`}
                            ...
                            >
                              <div className="flex items-start justify-between gap-2.5 sm:gap-3">
                                ...
                                <div className="flex shrink-0 flex-col items-end gap-1.5 sm:gap-2">
                                  {!isUnlocked ? (
                                    <span className="whitespace-nowrap rounded-full px-2.5 py-1 text-[10px] font-black sm:px-3 sm:text-[11px] bg-slate-100 text-slate-600 border border-slate-300">
                                      🔒 Bloccato
                                    </span>
                                  ) : (
                                    <button
                                      ...
                                      className={`px-3.5 py-1.5 rounded-full font-black text-xs sm:text-sm shadow-md transition-all active:scale-95 cursor-pointer flex items-center gap-1 animate-pulse ${
                                        isCompleted
                                          ? 'bg-emerald-500 hover:bg-emerald-600 text-white ring-2 ring-emerald-300'
                                          : 'bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-amber-950 ring-2 ring-amber-300'
                                      }`}
                                    >
                                      ENTRA ➔
                                    </button>
                              ...
                              {isUnlocked && (
                                <div className="mt-2.5 flex items-center gap-2 font-sans">
                                  <div className="flex items-center gap-1.5 rounded-xl bg-amber-100/90 px-2.5 py-1 text-amber-900 border border-amber-300/80 shadow-2xs">
                                    <span className="text-sm">🪙</span>
                                    <span className="font-black text-xs sm:text-sm">{worldProg.coins ?? (world.id === 2 ? profile.coins : 0)}</span>
                                    <span className="text-[10px] font-bold text-amber-800 uppercase tracking-wider">Monete</span>
                                  </div>
                                  <div className="flex items-center gap-1.5 rounded-xl bg-sky-100/90 px-2.5 py-1 text-sky-900 border border-sky-300/80 shadow-2xs">
                                    <span className="text-sm">💧</span>
                                    <span className="font-black text-xs sm:text-sm">{worldProg.lightDrops ?? (world.id === 2 ? profile.lightDrops : 0)}</span>
                                    <span className="text-[10px] font-bold text-sky-800 uppercase tracking-wider">Gocce</span>
                                  </div>
                                </div>
                              )}
                              ...
                              <div className="mt-3 grid grid-cols-[repeat(auto-fit,minmax(3.5rem,1fr))] gap-1.5 sm:mt-4 sm:gap-2">
                                {world.monuments.map(monument => {
                                  ...
                                  const canAfford = (worldProg.lightDrops ?? 0) >= monument.cost;
                                  return (
                                    <div
                                      key={monument.id}
                                      onClick={(e) => {
                              ...
                              <div className="mt-3 space-y-2 sm:mt-4">
                                ...
                                <div className="flex items-center justify-between gap-2 text-[10px] font-semibold text-sky-900/80 sm:text-[11px]">
                                  ...
                                  {!isUnlocked && <span className="shrink-0 font-bold text-slate-500">Completa il precedente</span>}
                                </div>
                              </div>
                            </div>
                          );
      ...
      {showAllKingdomsCelebrationModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                type="button"
                onClick={() => setShowAllKingdomsCelebrationModal(false)}
      ...
      {appMonumentModal && (() => {
        const mWorldProg = profile ? getAdventureWorldProgress(profile, appMonumentModal.world.id, devModeEnabled) : null;
        const currentWorldDrops = mWorldProg?.lightDrops ?? 0;
        const modalCanAfford = currentWorldDrops >= appMonumentModal.monument.cost;
            ...
            >
              {appMonumentModal.isErected ? (
                ...
                <>
                  ...
                  <p className="text-xs text-slate-600 mb-5 leading-relaxed">
                    Hai a disposizione <b>{currentWorldDrops} Gocce di Luce</b> in questo Regno. Vuoi spendere {appMonumentModal.monument.cost} Gocce per erigere questo monumento nel {appMonumentModal.world.title}?
                  </p>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        handleUpdateProfile(p => {
                          ...
                          const newDrops = Math.max(0, (worldProg.lightDrops ?? currentWorldDrops) - appMonumentModal.monument.cost);
                          const nextUnlocked = [...p.unlockedWorlds];
                          if ((worldProg.completedSteps?.includes('sfida') || worldProg.completedSteps?.length >= 6) && appMonumentModal.world.id + 1 <= 9 && !nextUnlocked.includes(appMonumentModal.world.id + 1)) {
                            nextUnlocked.push(appMonumentModal.world.id + 1);
                          }
                          return {
                            ...p,
                ...
                <>
                  ...
                  <div className="inline-flex items-center gap-1 text-xs font-black text-rose-900 bg-rose-100 border border-rose-200 px-3 py-1 rounded-full mb-3">
                    Costo: 💧 {appMonumentModal.monument.cost} (Ne hai {currentWorldDrops})
                  </div>
                  <p className="text-xs text-slate-600 mb-5 leading-relaxed">
                    Per erigere <b>{appMonumentModal.monument.name}</b> ti mancano <b>{appMonumentModal.monument.cost - currentWorldDrops} Gocce di Luce</b> in questo Regno.
                    <br /><br />
      ...
      <CurrencyInfoModal
        ...
        allMonumentsErected={WORLDS_DATA.every(world => {
          if (!profile) return false;
          const worldProg = getAdventureWorldProgress(profile, world.id, devModeEnabled);
          const rebuiltCount = devModeEnabled ? world.monuments.length : (worldProg?.rebuiltMonuments?.length || 0);
          return rebuiltCount >= world.monuments.length;
        })}
      />
    </div>
