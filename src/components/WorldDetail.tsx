...
const getSfidaDuration = (worldId: number) => Math.max(18, 32 - Math.floor((worldId - 1) * 1.5));
const COSTRUISCO_BALLOON_SPAWN_MIN_MS = 600;
const COSTRUISCO_BALLOON_SPAWN_MAX_MS = 1400;
const COSTRUISCO_BALLOON_FLIGHT_MIN_MS = 3800;
const COSTRUISCO_BALLOON_FLIGHT_MAX_MS = 5200;
const COSTRUISCO_BALLOON_MAX_ACTIVE = 4;
...
const COSTRUISCO_BALLOON_PALETTES = [
  {
    body: 'bg-gradient-to-b from-sky-300 via-sky-400 to-sky-600 text-white border-white hover:from-sky-400 hover:to-sky-700 shadow-sky-300/50',
    knot: 'bg-sky-700',
    string: 'bg-sky-400',
  },
  {
    body: 'bg-gradient-to-b from-fuchsia-300 via-pink-400 to-fuchsia-600 text-white border-white hover:from-fuchsia-400 hover:to-fuchsia-700 shadow-fuchsia-300/50',
    knot: 'bg-fuchsia-700',
    string: 'bg-fuchsia-400',
  },
  {
    body: 'bg-gradient-to-b from-amber-300 via-amber-400 to-orange-600 text-white border-white hover:from-amber-400 hover:to-orange-700 shadow-amber-300/50',
    knot: 'bg-orange-700',
    string: 'bg-amber-400',
  },
  {
    body: 'bg-gradient-to-b from-violet-300 via-purple-400 to-violet-600 text-white border-white hover:from-violet-400 hover:to-violet-700 shadow-violet-300/50',
    knot: 'bg-violet-700',
    string: 'bg-violet-400',
  },
  {
    body: 'bg-gradient-to-b from-emerald-300 via-teal-400 to-emerald-600 text-white border-white hover:from-emerald-400 hover:to-emerald-700 shadow-emerald-300/50',
    knot: 'bg-emerald-700',
    string: 'bg-emerald-400',
  },
  {
    body: 'bg-gradient-to-b from-rose-300 via-red-400 to-rose-600 text-white border-white hover:from-rose-400 hover:to-rose-700 shadow-rose-300/50',
    knot: 'bg-rose-700',
    string: 'bg-rose-400',
  },
  {
    body: 'bg-gradient-to-b from-cyan-300 via-cyan-400 to-blue-600 text-white border-white hover:from-cyan-400 hover:to-blue-700 shadow-cyan-300/50',
    knot: 'bg-blue-700',
    string: 'bg-cyan-400',
  },
  {
    body: 'bg-gradient-to-b from-yellow-200 via-yellow-400 to-amber-500 text-amber-950 border-white hover:from-yellow-300 hover:to-amber-600 shadow-yellow-300/50',
    knot: 'bg-amber-700',
    string: 'bg-yellow-400',
  },
  {
    body: 'bg-gradient-to-b from-indigo-300 via-indigo-500 to-indigo-800 text-white border-white hover:from-indigo-400 hover:to-indigo-900 shadow-indigo-400/50',
    knot: 'bg-indigo-900',
    string: 'bg-indigo-400',
  },
] as const;
...
export default function WorldDetail({ world, profile, updateProfile, onBack, compactLayout = false, initialExercise, devMode = false }: WorldDetailProps) {
  ...
  // Timer for completing table step exercises (comprendo, salto, costruisco, trucchi)
  const [stepTimerSeconds, setStepTimerSeconds] = useState<number>(0);

  useEffect(() => {
    const isGameActive =
      (activeStep === 'comprendo' && comprendoFlowStage === 'game' && !comprendoGameCompleted) ||
      (activeStep === 'salto' && saltoFlowStage === 'game' && !saltoGameCompleted) ||
      (activeStep === 'costruisco' && costruiscoFlowStage === 'game' && !costruiscoGameCompleted) ||
      (activeStep === 'trucchi' && trucchiFlowStage === 'game' && !trucchiQuestionSolved);
  ...
  const [sfidaTimer, setSfidaTimer] = useState<number>(() => getSfidaDuration(world.id));
  ...
  const [showKingdomErectedModal, setShowKingdomErectedModal] = useState(false);
  const [showAllKingdomsErectedModal, setShowAllKingdomsErectedModal] = useState(false);
  ...
  // Generate option pool with progressive difficulty based on current step index (0 to factor - 1)
  const generateSaltoOptions = (factor: number, stepIdx: number = saltoIndex) => {
    const expected = world.id * (stepIdx + 1);
    const optionsSet = new Set<number>([expected]);

    if (stepIdx === 0) {
      // Jump 1: 3 simple options, spaced out
      optionsSet.add(expected + world.id);
      optionsSet.add(expected + world.id * 2);
    } else if (stepIdx < 3) {
      // Jumps 2-3: 4 options with adjacent multiples
      if (expected - world.id > 0) optionsSet.add(expected - world.id);
      optionsSet.add(expected + world.id);
      optionsSet.add(expected + world.id * 2);
      if (expected - 2 * world.id > 0) optionsSet.add(expected - 2 * world.id);
    } else {
      // Jumps 4+: Progressive challenge (tricky distractors off by 1 or adjacent multiples)
      if (expected - 1 > 0) optionsSet.add(expected - 1);
      optionsSet.add(expected + 1);
      if (expected - world.id > 0) optionsSet.add(expected - world.id);
      optionsSet.add(expected + world.id);
      if (expected + 2 <= world.id * factor + 10) optionsSet.add(expected + 2);
    }

    const targetCount = stepIdx === 0 ? 3 : 4;
    let attempts = 0;
    while (optionsSet.size < targetCount && attempts < 100) {
      const delta = (Math.floor(Math.random() * 4) + 1) * (Math.random() < 0.5 ? 1 : -1);
      const candidate = expected + delta;
      if (candidate > 0 && candidate !== expected) {
  ...
  useEffect(() => {
    if (
      ...
      generateSaltoOptions(saltoSelectedFactor, saltoIndex);
    }
  }, [saltoSelectedFactor, activeStep, world.id, saltoFlowStage, saltoGameCompleted, saltoOptions.length, saltoIndex]);
  ...
  const startTrucchiGame = (factor: number | null = trucchiSelectedFactor) => {
  ...
  const generateCostruiscoBalloonPool = (worldId: number, factor: number): number[] => {
    ...
    // Guarantee 1 or 2 distractor balloons appear before the correct answer balloon
    const distractor1 = shuffledDistractors[0];
    const distractor2 = shuffledDistractors[1];
    const remainingDistractors = shuffledDistractors.slice(2);
    const shuffledLater = shuffleArray([correct, ...remainingDistractors]);

    return [distractor1, distractor2, ...shuffledLater].filter((v): v is number => v !== undefined);
  };
  ...
  const queueCostruiscoSpawn = (factor: number) => {
    ...
    const flightMin = Math.max(1600, COSTRUISCO_BALLOON_FLIGHT_MIN_MS - (world.id - 1) * 250);
    const flightMax = Math.max(2600, COSTRUISCO_BALLOON_FLIGHT_MAX_MS - (world.id - 1) * 300);
    const spawnMin = Math.max(300, COSTRUISCO_BALLOON_SPAWN_MIN_MS - (world.id - 1) * 35);
    const spawnMax = Math.max(650, COSTRUISCO_BALLOON_SPAWN_MAX_MS - (world.id - 1) * 70);

    const delayMs = randomInRange(spawnMin, spawnMax);
    costruiscoSpawnTimeoutRef.current = window.setTimeout(() => {
      ...
      const balloon: CostruiscoActiveBalloon = {
        ...
        flightMs: randomInRange(flightMin, flightMax),
  ...
  const handleQuizAnswer = (selectedVal: number) => {
    ...
    updateProfile(p => {
      ...
      const worldProg = p.worldProgress[world.id] || {
        ...
        highScore: 0,
        stars: 0,
        coins: world.id === 2 ? p.coins : 0,
        lightDrops: world.id === 2 ? p.lightDrops : 0,
      };
      let wCoins = worldProg.coins ?? (world.id === 2 ? p.coins : 0);
      let wDrops = worldProg.lightDrops ?? (world.id === 2 ? p.lightDrops : 0);
  ...
  const beginSfidaFromUnlockFlow = () => {
    const curWCoins = profile.worldProgress[world.id]?.coins ?? (world.id === 2 ? profile.coins : 0);
    if (curWCoins < SFIDA_UNLOCK_COST) {
      sound.playError();
    ...
    updateProfile(p => {
      ...
      const curCoins = worldProg.coins ?? (world.id === 2 ? p.coins : 0);
      return {
        ...p,
        coins: Math.max(0, p.coins - SFIDA_UNLOCK_COST),
        worldProgress: {
          ...p.worldProgress,
          [world.id]: {
            ...worldProg,
            coins: Math.max(0, curCoins - SFIDA_UNLOCK_COST),
          }
        }
      };
    });
  ...
  const initializeSfida = () => {
    ...
    setSfidaScore(0);
    setSfidaTimer(getSfidaDuration(world.id));
  ...
  const finishSfidaMode = (finalScore?: number) => {
    ...
    updateProfile(p => {
      ...
      // Unlock next world when passedSfida (>14) or completed Sfida!
      const nextUnlocked = [...p.unlockedWorlds];
      const nextWorldId = world.id + 1;
      if ((passedSfida || completed.includes('sfida')) && nextWorldId <= 9 && !nextUnlocked.includes(nextWorldId)) {
      ...
      const curWorldDrops = worldProg.lightDrops ?? (world.id === 2 ? p.lightDrops : 0);
      const nextWorldDrops = curWorldDrops + sfidaDropsEarned;
  ...
  const handleRebuildMonument = (monId: string, cost: number) => {
    const curDrops = profile.worldProgress[world.id]?.lightDrops ?? (world.id === 2 ? profile.lightDrops : 0);
    if (curDrops < cost) {
      sound.playError();
    ...
    updateProfile(p => {
      ...
      const newWDrops = Math.max(0, (worldProg.lightDrops ?? (world.id === 2 ? p.lightDrops : 0)) - cost);

      const nextUnlocked = [...p.unlockedWorlds];
      const nextWorldId = world.id + 1;
      const completed = worldProg.completedSteps || [];
      if ((completed.includes('sfida') || completed.length >= 6) && nextWorldId <= 9 && !nextUnlocked.includes(nextWorldId)) {
        nextUnlocked.push(nextWorldId);
      }

      const updatedProfile = {
        ...p,
        lightDrops: Math.max(0, p.lightDrops - cost),
        unlockedWorlds: nextUnlocked,
        worldProgress: {
          ...p.worldProgress,
          [world.id]: {
            ...worldProg,
            lightDrops: newWDrops,
            rebuiltMonuments: monuments
          }
        }
      };

      const isThisKingdomFullyErected = monuments.length >= world.monuments.length;
      const areAllKingdomsFullyErected = WORLDS_DATA.every(w => {
        const wProg = updatedProfile.worldProgress[w.id];
        const count = devMode ? w.monuments.length : (wProg?.rebuiltMonuments?.length || 0);
        return count >= w.monuments.length;
      });

      if (areAllKingdomsFullyErected) {
        setTimeout(() => {
          setShowFireworks(true);
          setShowAllKingdomsErectedModal(true);
          sound.playLevelUp();
          speak("Straordinario! Hai eretto tutti i regni di Tabellandia! Sei un campione leggendario!");
        }, 300);
      } else if (isThisKingdomFullyErected) {
        setTimeout(() => {
          setShowFireworks(true);
          setShowKingdomErectedModal(true);
          sound.playPowerUp();
          speak(`Evviva! Hai eretto tutti i monumenti nel Regno del ${world.id}!`);
        }, 300);
      }
  ...
  const worldProgBase = profile.worldProgress[world.id] || {
    ...
    highScore: 0,
    stars: 0,
    coins: world.id === 2 ? (profile.coins || 0) : 0,
    lightDrops: world.id === 2 ? (profile.lightDrops || 0) : 0,
  };
  const worldProg = devMode
    ? {
        ...worldProgBase,
        completedSteps: [...ALL_STEP_IDS],
        coins: worldProgBase.coins ?? (world.id === 2 ? (profile.coins || 0) : 0),
        lightDrops: worldProgBase.lightDrops ?? (world.id === 2 ? (profile.lightDrops || 0) : 0),
      }
    : {
        ...worldProgBase,
        coins: worldProgBase.coins ?? (world.id === 2 ? (profile.coins || 0) : 0),
  ...
  const worldCoins = worldProg.coins;
  const worldLightDrops = worldProg.lightDrops;
  ...
  const isKingdomRestored = (worldProg.rebuiltMonuments.length >= world.monuments.length) && isSfidaDone;
  ...
  // Auto scroll to active step or monuments in intro view
  useEffect(() => {
    if (activeStep === 'intro' && !currentView) {
      const scrollTarget = () => {
        const targetId = nextStepToPlay ? `active-step-card-${nextStepToPlay}` : 'active-monuments-section';
      ...
      const t3 = setTimeout(scrollTarget, 800);
      return () => {
        clearTimeout(t1);
        clearTimeout(t2);
        clearTimeout(t3);
  ...
  }, [activeStep, nextStepToPlay, currentView]);
  ...
  return (
    ...
    >
            ...
            {(isGuideIntroView || isGuideStoryView) && (
                <div className="rounded-3xl border border-indigo-200 bg-gradient-to-br from-indigo-50 via-white to-sky-50 p-6 text-slate-900 shadow-xl">
                  <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4">
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-300 text-[10px] font-black uppercase tracking-wider mb-2 shadow-2xs">
      ...
      <div className={`flex-1 overflow-y-auto flex flex-col ${compactLayout ? 'p-3' : 'p-4 md:p-6'}`}>
        {activeStep !== 'comprendo' && activeStep !== 'salto' && activeStep !== 'costruisco' && activeStep !== 'trucchi' && activeStep !== 'pratico' && activeStep !== 'sfida' && (
          <div className="max-w-3xl mx-auto w-full flex-1 flex flex-col justify-start gap-5">
            {/* Header del Sentiero con Monumenti inclusi */}
            <div className="bg-white/80 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-indigo-100 shadow-sm space-y-3.5">
              ...
              {isKingdomRestored && (
                <div className="rounded-2xl border-2 border-emerald-400 bg-gradient-to-r from-emerald-100 via-amber-100 to-emerald-200 p-3.5 text-center shadow-md">
                  <span className="text-2xl mb-1 block">🏛️</span>
                  <h4 className="text-sm font-black text-emerald-950 font-sans tracking-wide">
                    REGNO RIPRISTINATO!
                  </h4>
                  <p className="text-xs font-bold text-emerald-800 mt-0.5 font-sans">
                    Hai scacciato la nebbia e ricostruito tutti i monumenti del Regno del {world.id}!
                  </p>
                </div>
              )}

              <div role="list" className="grid grid-cols-[repeat(auto-fit,minmax(10rem,1fr))] gap-2.5">
                <button
                  type="button"
                  role="listitem"
              ...
              {/* MONUMENTI DEL REGNO INCLUSI NEL BOX SENTIERO */}
              <div id="active-monuments-section" className="pt-3 border-t border-indigo-100/80 space-y-2">
                <div className="flex items-center justify-between px-1">
                  <h4 className="text-xs font-black text-indigo-900 uppercase tracking-wider font-sans">
                    Erigi Monumenti ({rebuiltCount}/{world.monuments.length} Eretti)
                  </h4>
                </div>
                  ...
                  {world.monuments.map(monument => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-2 sm:p-3 rounded-2xl border-2 flex flex-col justify-between transition-all cursor-pointer ${
                          isErected
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            {isErected ? (
                              <span className="text-2xl sm:text-3xl filter drop-shadow-xs">{monument.emoji}</span>
                            ) : (
                              <div className="relative">
                                <span className={`text-2xl sm:text-3xl filter ${canAfford ? 'drop-shadow-md scale-110' : 'grayscale opacity-50'}`}>{monument.emoji}</span>
                                {!canAfford && <span className="absolute -top-1 -right-1 text-[10px]">🔒</span>}
                              </div>
                            )}

                            {isErected ? (
                              <span className="text-[9px] sm:text-[10px] font-black text-amber-950 bg-amber-300 border border-amber-400 px-1.5 sm:px-2 py-0.5 rounded-full font-sans shadow-2xs truncate">
                                ✓ ERETTO
                              </span>
                            ) : canAfford ? (
                              <span className="text-[9px] sm:text-[10px] font-black text-amber-950 bg-amber-300 border border-amber-500 px-1.5 sm:px-2 py-0.5 rounded-full font-sans shadow-md animate-badge-blink flex items-center gap-0.5 truncate">
                                ✨ SBLOCCA
                              </span>
                            ) : (
                              <span className="text-[9px] sm:text-[10px] font-black text-sky-900 bg-sky-100 border border-sky-200 px-1.5 sm:px-2 py-0.5 rounded-full font-sans truncate">
                                💧 {monument.cost}
                              </span>
                            )}
                          </div>
                          <h5 className={`text-[11px] sm:text-xs font-black font-sans mt-0.5 truncate ${isErected ? 'text-amber-950' : canAfford ? 'text-amber-950 font-black' : 'text-slate-700'}`}>
                            {monument.name}
                          </h5>
                          <p className={`text-[9px] sm:text-[10px] leading-tight mt-0.5 font-sans line-clamp-2 ${isErected ? 'text-amber-900/80' : canAfford ? 'text-amber-900 font-medium' : 'text-slate-500'}`}>
                            {monument.description}
                          </p>
                        </div>

                        {isErected ? (
                          <div className="mt-2 pt-1 border-t border-amber-200/80 text-[9px] sm:text-[10px] font-black text-emerald-800 font-sans flex items-center justify-between">
                            <span className="truncate">Attivo</span>
                          ...
                          <button
                            ...
                            className={`mt-2 w-full py-1.5 px-1 rounded-xl text-[10px] sm:text-xs font-black shadow-md cursor-pointer transition-all truncate ${
                              canAfford
                          ...
                          >
                            {canAfford ? `✨ SBLOCCA ORA` : `🔒 Servono ${monument.cost} 💧`}
                          </button>
                        )}
                      </div>
                    );
            ...
            <div className="space-y-2 order-2">
              <h4 className="text-xs font-black text-indigo-900 uppercase tracking-wider px-1 font-sans">
                Passi Didattici di Apprendimento
              </h4>
              <div className="grid grid-cols-1 gap-3">
                {[
                  { id: 'comprendo', title: '1. Comprendo', desc: 'Rappresentazione e gruppi.', icon: '🍎', isFactorBased: true },
                  { id: 'salto', title: '2. Salto', desc: 'Conteggio ritmico sul ruscello.', icon: '🐸', isFactorBased: true },
                  { id: 'costruisco', title: '3. Costruisco', desc: 'Griglia dei moltiplicatori.', icon: '🧱', isFactorBased: true },
                  { id: 'trucchi', title: '4. Trucchi', desc: 'Regole e scorciatoie cognitive.', icon: '🧠', isFactorBased: true },
                  { id: 'pratico', title: '5. Pratico (Avventura)', desc: 'Sconfiggi la nebbia e raccogli monete per la Sfida.', icon: '🛡️', isFactorBased: false },
                ].map((step, idx) => {
            ...
            <div className="space-y-2 order-3">
              ...
              {(() => {
                ...
                return (
                  <button
                    id="active-step-card-sfida"
                    type="button"
                    onClick={() => {
                      if (isSfidaLocked || isSfidaDone) {
                        if (isSfidaLocked) {
                          sound.playError();
                    ...
                    className={`w-full p-4 rounded-2xl border-2 flex flex-col gap-3 text-left transition-all ${
                      isSfidaLocked
                        ? 'opacity-60 bg-slate-100 border-slate-200 cursor-not-allowed'
                        : isSfidaDone
                          ? 'bg-gradient-to-r from-emerald-100 via-amber-50 to-emerald-50 border-emerald-400 shadow-md cursor-default'
                          : isSfidaNext
                            ? 'bg-gradient-to-r from-purple-600 to-indigo-600 border-amber-400 ring-4 ring-amber-300 text-white shadow-xl animate-pulse cursor-pointer'
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 w-full">
                      <div className="flex items-center gap-2.5">
                        <span className="text-2xl sm:text-3xl">{isSfidaLocked ? '🔒' : '⚡'}</span>
                        <h5 className={`text-sm sm:text-base font-black font-sans ${isSfidaLocked ? 'text-slate-700' : isSfidaDone ? 'text-emerald-950' : 'text-white'}`}>
                          6. Sfida Finale (Cronometro)
                        </h5>
                      </div>
                      <span className={`${isSfidaDone ? 'bg-sky-100 text-sky-900 border-sky-300' : 'bg-sky-400/30 text-sky-100 border-sky-300/40'} px-2.5 py-1 rounded-lg border text-[10px] sm:text-xs font-black font-sans shrink-0`}>
                        💧 Premio: +15~30 Gocce
                      </span>
                    </div>
                    ...
                    <div className="w-full">
                      {isSfidaDone ? (
                        <div className="w-full text-center text-xs sm:text-sm font-black py-2.5 px-4 rounded-xl shadow-xs font-sans bg-emerald-200 text-emerald-950 border border-emerald-300">
                          ✓ SFIDA SUPERATA
                        </div>
                      ) : (
                        <div className={`w-full text-center text-xs sm:text-sm font-black py-2.5 px-4 rounded-xl shadow-xs font-sans transition-all ${
                          isSfidaLocked
                            ? 'bg-slate-200 text-slate-600'
                            : 'bg-amber-400 text-amber-950 hover:bg-amber-300 shadow-md'
                        }`}>
                          ▶ Avvia Sfida ({SFIDA_UNLOCK_COST} 🪙)
                        </div>
                      )}
                    </div>
                  </button>
               ...
               <div className={`${comprendoFlowStage === 'game' ? 'max-w-2xl' : 'max-w-xl'} mx-auto w-full space-y-6`}>
                 ...
                 {comprendoFlowStage === 'game' && (
                   <div className="relative bg-white rounded-3xl p-5 border border-indigo-100 shadow-xl space-y-6">
                     <div className="flex items-center justify-between bg-indigo-50/80 rounded-2xl p-3 border border-indigo-100">
                       <span className="text-xs font-bold text-indigo-700 font-sans">
                         Comprendo: {world.id} × {comprendoSelectedFactor}
                       </span>
                       <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white border border-indigo-200 text-slate-700 text-xs font-bold shadow-2xs">
                         <Timer className="w-3.5 h-3.5 text-indigo-500 animate-pulse" />
                         <span>Tempo: {Math.floor(stepTimerSeconds / 60)}:{String(stepTimerSeconds % 60).padStart(2, '0')}</span>
                       </div>
                     </div>
                     <ComprendoBasketGame
                       a={world.id}
                     ...
                     {showComprendoCompletionEffect && (
                       ...
                       >
                         <div className="rounded-2xl border-2 border-emerald-300 bg-white/95 px-6 py-4 text-center shadow-xl">
                           ...
                           <p className="text-xs text-slate-500 mt-1 flex items-center justify-center gap-1 font-bold">
                             <Timer className="w-3.5 h-3.5 text-indigo-500" />
                             Tempo impiegato: {Math.floor(stepTimerSeconds / 60)}m {stepTimerSeconds % 60}s
                           </p>
                           <button
                             type="button"
                             onClick={() => {
                ...
                {saltoFlowStage === 'game' && (
                  <div className={`relative bg-white rounded-3xl border border-purple-100 shadow-xl ${compactLayout ? 'p-3 space-y-3' : 'p-5 space-y-5'}`}>
                    ...
                    <div className="flex items-center justify-between bg-gradient-to-r from-purple-50 to-indigo-50 p-3 rounded-2xl border border-purple-200 shadow-sm">
                      ...
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white border border-purple-200 text-slate-700 text-xs font-bold shadow-2xs">
                        <Timer className="w-3.5 h-3.5 text-purple-500 animate-pulse" />
                        <span>Tempo: {Math.floor(stepTimerSeconds / 60)}:{String(stepTimerSeconds % 60).padStart(2, '0')}</span>
                      </div>
                    </div>
               ...
               <div className="max-w-2xl mx-auto w-full space-y-6">
                 ...
                 {costruiscoFlowStage === 'game' && (
                   <div className="relative bg-white rounded-3xl p-5 border border-emerald-100 shadow-xl space-y-6">
                     <div className="flex items-center justify-between bg-emerald-100 rounded-2xl p-4 border-2 border-emerald-300">
                       ...
                       <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white border border-emerald-300 text-slate-700 text-xs font-bold shadow-2xs">
              ...
              <div className="max-w-xl mx-auto w-full space-y-5">
                ...
                {trucchiFlowStage === 'game' && (
                  <div className="relative min-h-[25rem] bg-white rounded-3xl border border-amber-100 shadow-xl p-4 sm:p-5 space-y-5">
                    <div className="flex items-center justify-between bg-amber-50 rounded-2xl p-3 border border-amber-200">
                      ...
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white border border-amber-300 text-slate-700 text-xs font-bold shadow-2xs">
          ...
          <div className="max-w-xl mx-auto w-full bg-white rounded-3xl p-5 sm:p-6 border border-indigo-100 shadow-xl space-y-5 flex flex-col items-center justify-center">
            <div className="text-center space-y-1.5 w-full">
              <div className="flex items-center justify-center gap-2">
                <span className="text-3xl sm:text-4xl" aria-hidden="true">{world.itemsToCount}</span>
                <h2 className="text-xl sm:text-2xl font-black text-indigo-950">SFIDA DEL {world.name.toUpperCase()}</h2>
                <button
              ...
              <p className="text-sm text-slate-600">Aiuta {world.mascotName} a proteggere la {world.locationName}! Risolvi più operazioni prima che il tempo finisca.</p>
            ...
            <button
              ...
              id="sfida-start-btn"
            >
              <span className="text-xs font-bold text-amber-100 uppercase tracking-widest">{getSfidaDuration(world.id)} secondi</span>
      ...
      {/* Modal Celebrazione Singolo Regno Eretto */}
      {showKingdomErectedModal && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <FireworksOverlay onDone={() => {}} />
          <motion.div
            initial={{ scale: 0.85, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-gradient-to-b from-amber-50 via-white to-emerald-50 rounded-3xl p-6 max-w-sm w-full shadow-2xl border-4 border-amber-400 text-center relative font-sans"
            role="dialog"
            aria-modal="true"
          >
            ...
            <h3 className="text-xl font-black text-slate-900 mb-1">
              REGNO DEL {world.id} RICOSTRUITO!
            </h3>
            <p className="text-xs font-bold text-emerald-700 mb-3">
              Hai eretto tutti i monumenti di {world.locationName}!
            </p>
            <p className="text-xs text-slate-600 mb-5 leading-relaxed bg-white/80 p-3 rounded-2xl border border-amber-200 text-left">
              Le magiche creature di questo regno festeggiano la tua vittoria! Tutti i monumenti risplendono di luce e la nebbia è sparita da queste terre.
            </p>
            <div className="flex gap-2.5">
              <button
                type="button"
                ...
                className="flex-1 py-2.5 rounded-xl bg-amber-400 hover:bg-amber-500 text-amber-950 font-black text-xs shadow-md cursor-pointer transition-transform active:scale-95"
              >
                🎆 Fuochi!
              </button>
              <button
                type="button"
                onClick={() => setShowKingdomErectedModal(false)}
                className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs shadow-md cursor-pointer transition-transform active:scale-95"
              >
                🚀 Continua
              </button>
            </div>
      ...
      {/* Modal Celebrazione Tutti i Regni Eretti */}
      {showAllKingdomsErectedModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
          <FireworksOverlay onDone={() => {}} />
          <motion.div
            initial={{ scale: 0.85, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="bg-gradient-to-b from-amber-50 via-white to-sky-50 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl border-4 border-amber-400 text-center relative font-sans my-8"
            role="dialog"
            aria-modal="true"
          >
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight mb-2">
              TUTTI I REGNI SONO STATI ERETTI!
            </h2>
            <p className="text-sm font-bold text-emerald-700 mb-4">
              🎆 Hai ricostruito ogni monumento in tutti e 9 i Regni di Tabellandia! 🏛️
            </p>

            <div className="bg-white/80 rounded-2xl p-4 border border-amber-200/80 text-left text-xs sm:text-sm text-slate-700 leading-relaxed space-y-2.5 mb-6 shadow-xs">
              <p>
                <strong className="text-amber-900 font-black">Eroe Straordinario,</strong> hai compiuto un'opera meravigliosa!
              </p>
              <p>
                Grazie alla tua costanza, al tuo impegno quotidiano e alla tua grandissima intelligenza, tutti e 9 i Regni risplendono di luce pura e la nebbia del buio è scomparsa per sempre.
              </p>
              <p>
                Hai dimostrato che con l'allenamento e la determinazione non esiste alcuna sfida o tabellina insuperabile. La tua mente è ora velocissima e invincibile!
              </p>
              <p className="font-bold text-sky-900 pt-1">
                🌟 Continua sempre a esplorare, imparare e superare ogni tuo limite. Sei un vero CAMPIONE LEGENDARIO DELLE TABELLINE!
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                onClick={() => {
                  sound.playPowerUp();
                  setShowFireworks(true);
                  speak("Complimenti straordinari! Hai eretto tutti i regni di Tabellandia!");
                }}
                className="flex-1 py-3 px-4 rounded-2xl bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-amber-950 font-black text-sm shadow-md transition-all active:scale-95 cursor-pointer flex items-center justify-center gap-2 border border-amber-300"
              >
                <span>🎆 Fuochi d'Artificio!</span>
              </button>
              <button
                type="button"
                onClick={() => setShowAllKingdomsErectedModal(false)}
                className="flex-1 py-3 px-4 rounded-2xl bg-sky-600 hover:bg-sky-700 text-white font-black text-sm shadow-md transition-all active:scale-95 cursor-pointer flex items-center justify-center gap-2"
              >
      ...
      <CurrencyInfoModal
        ...
        coins={profile.coins}
        allMonumentsErected={WORLDS_DATA.every(w => {
          const wProg = profile.worldProgress[w.id];
          const count = devMode ? w.monuments.length : (wProg?.rebuiltMonuments?.length || 0);
          return count >= w.monuments.length;
        })}
      />
