...
const createInitialParticles = (count: number, arena: ArenaSize, worldId: number = 1): BasketParticle[] => {
  ...
  const speedBoost = 1 + (worldId * 0.12);
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      vx: (Math.random() > 0.5 ? 1 : -1) * (45 + ((index * 13) % 22)) * speedBoost,
      vy: (Math.random() > 0.5 ? 1 : -1) * (40 + ((index * 15) % 20)) * speedBoost,
    };
  });
};
...
export default function ComprendoBasketGame({ a: propA, b: propB, itemEmoji, worldId = 1, onCompletionChange }: ComprendoBasketGameProps) {
  ...
  useEffect(() => {
    ...
    const nextParticles = prefersReducedMotion
      ...
      : createInitialParticles(a, arenaSize, worldId);
