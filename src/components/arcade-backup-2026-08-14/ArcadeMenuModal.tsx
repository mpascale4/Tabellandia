...
export type GameId = 'snake' | 'breakout' | 'whack' | 'pong' | 'flappy' | 'dino' | 'invaders' | 'memory' | 'simon';
...
export const ARCADE_GAMES: Array<{ id: GameId; emoji: string; name: string }> = [
  ...
  { id: 'breakout', emoji: '🧱', name: 'Breakout' },
  { id: 'whack', emoji: '🔨', name: 'Acchiappa la Talpa' },
  { id: 'pong', emoji: '🏓', name: 'Pong' },
  { id: 'flappy', emoji: '🐤', name: 'Flappy' },
  { id: 'dino', emoji: '🦖', name: 'Dino Run' },
  { id: 'invaders', emoji: '👾', name: 'Invasori Spaziali' },
  { id: 'memory', emoji: '🧠', name: 'Memory' },
  { id: 'simon', emoji: '🎵', name: 'Simon' },
];

function renderGame(id: GameId, onExit: () => void) {
  switch (id) {
    case 'snake': return <SnakeGame onExit={onExit} />;
    case 'breakout': return <BreakoutGame onExit={onExit} />;
    case 'whack': return <WhackAMoleGame onExit={onExit} />;
    case 'pong': return <PongGame onExit={onExit} />;
    case 'flappy': return <FlappyGame onExit={onExit} />;
    case 'dino': return <DinoGame onExit={onExit} />;
    case 'invaders': return <InvadersGame onExit={onExit} />;
    case 'memory': return <MemoryGame onExit={onExit} />;
    case 'simon': return <SimonGame onExit={onExit} />;
    default: return null;
  }
}
 ...
 */
export default function ArcadeMenuModal({ onExit, onlyGame }: ArcadeMenuModalProps) {
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl border-2 border-indigo-300 shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        ...
        <div className="p-4 sm:p-5 overflow-y-auto bg-slate-50 flex-1 flex flex-col gap-4">
          {selected === null ? (
            ...
            renderGame(selected, handleGameExit)
          )}
          {selected === null && (
            <button
              type="button"
